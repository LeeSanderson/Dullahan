/*
  ...
  Changes: 
    ...
    Renamed System.h to Sys.h as System is a reserved word.
	Removed utils.h and phantom.h
	Added CookieJar and Config to WebPage constructor so that we can use page instance jar rather than global instance
	Removed ownsPages/setOwnsPages property - WebPage now always owns pages (so they are cleaned up correctly).
*/
...
/*
  Default constructor - creates a "root" web page that owns a cookie jar and config objects that
  are shared amongst child pages.
*/
WebPage::WebPage()
	: QObject(0)
    ...
    , m_loadingProgress(0)
{
	CookieJar *cookieJar = new CookieJar(QString(), this);
	Config *config = new Config(this);

	init(cookieJar, config, QUrl());
}

/*
  Creates a child page belonging to a parent page 
*/
WebPage::WebPage(QObject *parent, CookieJar *cookieJar, Config * config, const QUrl &baseUrl)
    : QObject(parent)
    , m_navigationLocked(false)
    , m_mousePos(QPoint(0, 0))
    //, m_ownsPages(true)
    , m_loadingProgress(0)
{
	init(cookieJar, config, baseUrl);
}

void WebPage::init(CookieJar *cookieJar, Config * config, const QUrl &baseUrl)
{
    ...
    setLibraryPath(QFileInfo(config->scriptFile()).dir().absolutePath());
    ...
    if (config->offlineStoragePath().isEmpty()) {
        ...
        m_customWebPage->settings()->setOfflineStoragePath(config->offlineStoragePath());
    }
    if (config->offlineStorageDefaultQuota() > 0) {
        m_customWebPage->settings()->setOfflineStorageDefaultQuota(config->offlineStorageDefaultQuota());
    }
	...
	m_networkAccessManager = new NetworkAccessManager(this, config, cookieJar);
	...
	// Apply default page settings from config.
	// TODO: Alter applySettings to work directly from m_config.
	QVariantMap pageSettings;
	pageSettings[PAGE_SETTINGS_LOAD_IMAGES] = QVariant::fromValue(m_config->autoLoadImages());
    pageSettings[PAGE_SETTINGS_JS_ENABLED] = QVariant::fromValue(true);
    pageSettings[PAGE_SETTINGS_XSS_AUDITING] = QVariant::fromValue(false);
    pageSettings[PAGE_SETTINGS_USER_AGENT] = QVariant::fromValue(userAgent());
    pageSettings[PAGE_SETTINGS_LOCAL_ACCESS_REMOTE] = QVariant::fromValue(m_config->localToRemoteUrlAccessEnabled());
    pageSettings[PAGE_SETTINGS_WEB_SECURITY_ENABLED] = QVariant::fromValue(m_config->webSecurityEnabled());
    pageSettings[PAGE_SETTINGS_JS_CAN_OPEN_WINDOWS] = QVariant::fromValue(m_config->javascriptCanOpenWindows());
    pageSettings[PAGE_SETTINGS_JS_CAN_CLOSE_WINDOWS] = QVariant::fromValue(m_config->javascriptCanCloseWindows());
...
bool WebPage::setCookies(const QVariantList &cookies)
{
    ...
    return /*CookieJar::instance()*/m_cookieJar->addCookiesFromMap(cookies, this->url());
}

QVariantList WebPage::cookies() const
{
    // Return all the Cookies visible to this Page, as a list of Maps (aka JSON in JS space)
    return /*CookieJar::instance()*/m_cookieJar->cookiesToMap(this->url());
}

bool WebPage::addCookie(const QVariantMap &cookie)
{
    return /*CookieJar::instance()*/m_cookieJar->addCookieFromMap(cookie, this->url());
}

bool WebPage::deleteCookie(const QString &cookieName)
{
    if (!cookieName.isEmpty()) {
        return /*CookieJar::instance()*/m_cookieJar->deleteCookie(cookieName, this->url());
    }
    return false;
}
...
{
    return /*CookieJar::instance()*/m_cookieJar->deleteCookies(this->url());
}
...
bool WebPage::render(const QString &fileName, const QVariantMap &option)
{
	...
	/* LJS - exportGif not found?? Are we missing a library?
    else if ( format == "gif" ) {
	...
	*/
    ...
    if( tempFileName != "" ){
		...
		Sys *system =  this->createSystem(); // (Sys*)Phantom::instance()->createSystem();
...
Sys *WebPage::createSystem()
{
    if (!m_system) {
        m_system = new Sys(this);
    }

    return m_system;
}

#include "gen\moc_WebPage.cpp"
