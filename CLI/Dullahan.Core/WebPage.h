/*
  ...
  Changes: 
    Removed Phantom and QWebInspector classe references
*/
...
public:
	WebPage();
    WebPage(QObject *parent, CookieJar *cookieJar, Config * config, const QUrl &baseUrl = QUrl());
    virtual ~WebPage();
...
private:
	void init(CookieJar *cookieJar, Config * config, const QUrl &baseUrl = QUrl());
    QImage renderImage();
